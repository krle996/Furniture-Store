...
const office = {
    ...
    actions: {
        getTables({ commit }, payload) {
            Vue.http.get(`kancelarija/kancelarijski_stolovi.json?orderBy="$key"&limitToLast=${payload.limit}`)
        ...
        getTablePost({ commit }, payload) {
            Vue.http.get(`kancelarija/kancelarijski_stolovi.json?orderBy="$key"&equalTo="${payload}"`)
                .then(response => response.json())
                .then(responseData => {
                    let post = {};
                    for (let key in responseData) {
        ...
        insertTable({ commit, state }, payload) {
            Vue.http.post(`kancelarija/kancelarijski_stolovi.json`, payload)
                .then(response => {
                    let newItems = [];
        ...
        deleteTable({ commit, state }, payload) {
            Vue.http.delete(`kancelarija/kancelarijski_stolovi/${payload}.json`)
                .then(response => {
                    let newItems = [];
                    state.tables.forEach(item => {
                        if (item.id != payload) {
        ...
        getChairs({ commit }, payload) {
            Vue.http.get(`kancelarija/kancelarijske_stolice.json?orderBy="$key"&limitToLast=${payload.limit}`)
                .then(response => response.json())
                .then(responseData => {
                    const items = [];
                    for (let key in responseData) {
        ...
        getChairPost({ commit }, payload) {
            Vue.http.get(`kancelarija/kancelarijske_stolice.json?orderBy="$key"&equalTo="${payload}"`)
                .then(response => response.json())
                .then(responseData => {
                    let post = {};
                    for (let key in responseData) {
        ...
        insertChair({ commit, state }, payload) {
            Vue.http.post(`kancelarija/kancelarijske_stolice.json`, payload)
                .then(response => {
                    let newItems = [];
        ...
        deleteChair({ commit, state }, payload) {
            Vue.http.delete(`kancelarija/kancelarijske_stolice/${payload}.json`)
                .then(response => {
                    let newItems = [];
                    state.chairs.forEach(item => {
                        if (item.id != payload) {

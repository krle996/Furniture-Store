      ...
      <div class="row">
        ...
        >
          <div class="card my-3">
            <div class="card-body">
              <router-link
                :to="`/proizvodi/kancelarija/stolovi/${item.id}`"
                class="link"
                tag="div"
              >
...
export default {
  ...
  computed: {
    tables() {
      return this.$store.getters["office/getTables"];
    },
    isAdmin() {
  ...
  created() {
    this.$store.dispatch("office/getTables", {
      limit: 30,
    });
  },
  methods: {
    ...
    insertTable() {
      if (!this.$v.$invalid) {
        ...
        this.$store.dispatch("office/insertTable", this.formData);
        this.clearModal();
    ...
    deleteTable(id) {
      this.$store.dispatch("office/deleteTable", id);
    },
    clearImageUpload() {

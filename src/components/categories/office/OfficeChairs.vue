      ...
      <div class="row">
        ...
        >
          <div class="card my-3">
            <div class="card-body">
              <router-link
                :to="`/proizvodi/kancelarija/stolice/${item.id}`"
                class="link"
                tag="div"
              >
...
export default {
  ...
  computed: {
    chairs() {
      return this.$store.getters["office/getChairs"];
    },
    isAdmin() {
  ...
  created() {
    this.$store.dispatch("office/getChairs", {
      limit: 30,
    });
  },
  methods: {
    ...
    insertChair() {
      if (!this.$v.$invalid) {
        ...
        this.$store.dispatch("office/insertChair", this.formData);
        this.clearModal();
    ...
    deleteChair(id) {
      this.$store.dispatch("office/deleteChair", id);
    },
    clearImageUpload() {
